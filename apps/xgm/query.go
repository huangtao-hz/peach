...
func (c *Client) show_new_jy(jym string) {
	c.PrintRow("select jym,jymc,min(tcrq),zs,bz from xjdz where jym=? group by jym", xjyHeader, jym)
	fmt.Println("                      --  对应老交易清单  --")
	c.Printf(XjyQuery, "%4s  %-30s %-12s  %-12s  %12s    %10s\n", XyjOldHeader, true, jym)
}

func (c *Client) show_old_jy(jym string) {
	var fa, zt, jhbb string
	err := c.QueryRow("select fa,sfwc,ifnull(jhbb,'') from xmjh a left join kfjh b on a.jym=b.jym where a.jym=?", jym).Scan(&fa, &zt, &jhbb)
	if err != nil {
		fmt.Printf("交易 %s 不存在\n", jym)
		return
	}
...
func (c *Client) show_jhbb(jhbb string) {
	const (
...
order by a.jym`
	...
	c.Printf(query, format, header, true, jhbb)
}
...
func (c *Client) show_touchan(string) (err error) {
	header := "投产日期   交易数量 迁移交易数量 新交易数量    占比（%）"
	format := "%10s  %8,d  %8,d  %8,d        %5.2f %%\n"
	c.Printf(touchan_query, format, header, true)
	return
}
...
func (c *Client) kaifajihua(string) (err error) {
	header := "计划版本 交易数量        占比"
	format := "%-10s  %5,d      %5.2f%\n"
	c.Printf(kaifajihua_query, format, header, true)
	return
}
