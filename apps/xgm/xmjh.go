...
// load_xmjh 导入项目计划
func (c *Client) load_xmjh(file utils.File) (err error) {
	...
	if f, err = excel.Open(file); err == nil {
		...
		c.LoadExcel(loaderFS, "loader/jh_xjdzb.toml", &f.ExcelBook, fileinfo, date)
		fmt.Println("导入开发计划表")
		c.LoadExcel(loaderFS, "loader/jh_kfjh.toml", &f.ExcelBook, fileinfo, date)
		fmt.Println("导入项目计划表")
...
func (c *Client) PrintVersion() {
	var ver string
	if err := c.QueryRow("select ver from loadfile where name='xmjh'").Scan(&ver); err == nil {
...
func (c *Client) update_xmjh() (err error) {
	...
	r, _ := c.Exec(`update bbap set wcys=date(tcrq,"weekday 5","-7 days") where wcys=""`)
	if count, err := r.RowsAffected(); err == nil {
		fmt.Println(count, "条数据被更新")
	}
	fmt.Print("根据验收明细表更新开发状态:")
	c.ExecuteFs(queryFS, "query/update_kfjihua.sql")
	fmt.Print("根据计划版本更新开发计划时间：")
	c.ExecuteFs(queryFS, "query/update_kfjhsj.sql")
	fmt.Print("根据验收条目更新完成状态：")
	c.ExecuteFs(queryFS, "query/update_xmjh.sql")
